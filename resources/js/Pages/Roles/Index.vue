            ...
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="entry in props.roles.data" :key="entry.id" class="text-gray-700">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <PrimaryLink v-if="entry.deleted_at == null && $page.props.user.permissions.includes('edit role') " :href="route('roles.edit', { 'id': entry.id })"
                                class="max-w-xl ml-2">EDIT
                            </PrimaryLink>
                            <DangerButton class="ml-3" @click="deleteTrade(entry.id)" v-if="entry.deleted_at == null && $page.props.user.permissions.includes('delete role')">
                                Trash
                            </DangerButton>
                        </td>
                    </tr>
