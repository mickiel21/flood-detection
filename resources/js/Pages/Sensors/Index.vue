...
const deleteTrade = (id) => {
    if (confirm("Are you sure you want to move this to trash")) {
        form.delete(route('sensors.destroy', { id: id }), {
            preserveScroll: true,
        });
    }
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight"><a
                    :href="route('sensors.index')">Sensors</a>
                <PrimaryLink :href="route('sensors.create')" class="max-w-xl ml-2">Add Sensor</PrimaryLink>
            </h2>
        </template>
        ...
        <div class="inline-block min-w-full overflow-x-auto rounded-lg shadow">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="entry in props.sensors.data" :key="entry.id" class="text-gray-700">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <PrimaryLink v-if="entry.deleted_at == null && $page.props.user.permissions.includes('edit sensor') " :href="route('sensors.edit', { 'id': entry.id })"
                                class="max-w-xl ml-2">EDIT
                            </PrimaryLink>
                            <DangerButton class="ml-3" @click="deleteTrade(entry.id)" v-if="entry.deleted_at == null && $page.props.user.permissions.includes('delete sensor')">
                                Trash
                            </DangerButton>
                        </td>
                    </tr>
                    <tr v-if="props.sensors.data.length === 0">
                        <td class="px-6 py-4 border-t" colspan="4">No posts found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        <pagination class="mt-6" :links="props.sensors.links" />
    </AuthenticatedLayout>
</template>